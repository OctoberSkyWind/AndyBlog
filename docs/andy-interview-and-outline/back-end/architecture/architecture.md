...
#### update_time字段的用法和注意
- 用法：
  1. 记录数据更新时间，用于并发控制（乐观锁）与排错依据
  2. 实现缓存失效策略，确保数据一致性（做缓存是否失效的校验字段）
  3. 处理乱序消息与幂等性，优化异步数据同步流程
  4. 增量ETL依据
  5. MySQL同步到ES等范围依据
  6. HTTP缓存协商：前端加一个类似"LastModified"的请求头，后端校验如果没有修改，返回约定Code，前端直接复用本地缓存的文件（大文本情况非常适用）
  7. 冷热分类依据
- 注意点：
  1. 数据库托管update_time字段，建议不要自己写update_time字段，因为服务器时间可能不一致
  2. 可以与软删除联动（删除时间就是update_time字段）
  3. 存储格式建议统一采用UTC或者Unix时间戳，强一致系统建议精确到微秒级别